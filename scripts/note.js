...
$clear.addEventListener('click', (event) => {
    ...
    // First, we remove all the notes from the HTML...
    notesArray = []
...
$add.addEventListener('click', (event) => {
    ...
    // getMonth() gives us an integer from 0 - 11, because reasons, apparently. Tsk tsk, silly programmers... So January 6th would be 6/0/2025. We we need to add 1 to the month to account for this.
...
function search() {
    ...
    // For these notes, we want to highlight them, so we also provide true as the second parameter. This is defined below where we make the function.
...
// Here we have a helper function for displaying all the notes in a given array. Havin this helps us avoid including unnecessary code in the other functions and event listeners. The exact details of how notes are displayed isn't relevant there, so it's nice to abstract it away behind a clearly named function, like this.
function displayNotes(array, highlight = false) {
    // First we reset the $notes div, then we just cycle through the notes, adding them to the page in sequence.
    $notes.innerHTML = ''
    array.forEach(note => {
        // We have the ability to indicate if we want the notes to be highlighted too.
        displayNote(note, highlight)
    })
}


// Now we have our code for displaying a given note.
